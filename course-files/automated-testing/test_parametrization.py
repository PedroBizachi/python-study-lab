...
def test_is_valid_dash():
    assert is_valid_hostname_char("-") is True


def test_is_valid_upper_case():
...
@pytest.mark.parametrize(
    ...
    [("a", True), ("5", True), ("-", True), ("A", False), ("_", False)],
)
def test_is_valid_hostname_char(input_char: str, expected_result: bool):
...
def test_is_valid_hostname_custom_param(input_char: str, expected_result: bool):
    assert is_valid_hostname_char(input_char) is expected_result


@pytest.mark.parametrize(
    "url_to_check, expected_status_code, expected_message",
    [
...
def test_various_url_statuses(
    url_to_check: str, expected_status_code: int, expected_message: str
):
    status_code, message = check_url_status(url_to_check)
