...
from dummy_functions import check_file_exists, fetch_both_endpoints, get_current_user
...
def test_get_current_user_command_fails(mocker: MockerFixture):
    ...
    mock_run.side_effect = subprocess.CalledProcessError(returncode=1, cmd=["whoami"])
...
def test_check_file_exists_side_effect_list(mocker: MockerFixture):
    mock_exists = mocker.patch(
        "dummy_functions.os.path.exists", side_effect=[True, False]
    )

    assert check_file_exists("some/path/one.txt") is True
    assert check_file_exists("some/path/two.txt") is False

    assert mock_exists.call_count == 2
    assert [call.args[0] for call in mock_exists.call_args_list] == [
        "some/path/one.txt",
        "some/path/two.txt",
    ]
...
@pytest.mark.xfail(reason="Context managers do not work with Mock")
def test_context_manager_with_mock(mocker: MockerFixture):
    fake_cm = mocker.Mock()
    fake_cm.__enter__.return_value = fake_cm
    fake_cm.read.return_value = "some data"
...
def test_context_manager_with_magic_mock(mocker: MockerFixture):
