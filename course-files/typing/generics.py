...
def get_first_item(input_list: list[T]) -> Optional[T]:
    if input_list:
        return input_list[0]
...
first_item = get_first_item(["1", "2"])
first_number = get_first_item([1, 2])
first_mixed = get_first_item([1, 2, "3"])
...
sum_int = add_generic_numbers(3, 0.5)
...
def filter_deployed(resources: list[ResourceType]) -> list[ResourceType]:
